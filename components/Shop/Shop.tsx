...
const Shop: React.FC<ShopProps> = ({ isOpen, onClose }) => {
  ...
  const getUpgradeIcon = (upgradeId: string) => {
    switch (upgradeId) {
      ...
      case "magnet":
        return <Magnet className="w-6 h-6 text-cyan-400" />;
      case "shield":
  ...
  const getEffectDisplay = (upgradeId: string, level: number) => {
    ...
    switch (upgradeId) {
      ...
      case "magnet":
        return `%${Math.round(effect * 100)} çekim alanı`;
      case "shield":
        return `${effect} kalkan / koşu`;
      case "score-multiplier":
...
const SkinPreview: React.FC<{ skin: BallSkin }> = ({ skin }) => {
  const getOrbStyle = () => {
    if (skin.type === "gradient" && skin.config.gradient) {
      return {
        ...
        boxShadow: skin.config.glowColor
          ? `0 0 10px ${skin.config.glowColor}`
          : undefined,
      };
    }
    if (skin.type === "solid") {
  ...
  return <div className="w-10 h-10 rounded-full" style={getOrbStyle()} />;
};
...
const ThemePreview: React.FC<{ theme: Theme }> = ({ theme }) => {
  return (
    <div className="w-full h-12 rounded-lg overflow-hidden flex flex-col">
      <div className="flex-1" style={{ backgroundColor: theme.colors.topBg }} />
      <div
        className="flex-1"
