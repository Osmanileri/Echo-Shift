...
const GameEngine: React.FC<GameEngineProps> = ({ 
  ...
  const midlineState = useRef<MidlineState>(createInitialMidlineState(window.innerHeight));
  ...
  const slowMotionState = useRef<SlowMotion.SlowMotionState>(SlowMotion.createInitialSlowMotionState(0));
  ...
  const zenModeState = useRef<ZenMode.ZenModeState>(ZenMode.createInitialZenModeState());

  // Ghost Racer State - Requirements 15.1, 15.2, 15.3, 15.4
  const ghostRacerState = useRef<GhostRacer.GhostRacerState>(GhostRacer.createInitialGhostRacerState());
  ...
  const resonanceState = useRef<ResonanceSystem.ResonanceState>(ResonanceSystem.createInitialResonanceState());

  // S.H.I.F.T. Protocol State - Requirements 3.2, 3.3, 9.1
  const shiftState = useRef<ShiftProtocolState>(ShiftProtocol.initializeShiftState());
  ...
  const snapshotBuffer = useRef<SnapshotBuffer>(RestoreSystem.initializeSnapshotBuffer());
  ...
  const joystickRef = useRef<{ active: boolean; startY: number; currentY: number }>({
    active: false,
    startY: 0,
    currentY: 0
  });
  ...
  const resetGame = useCallback(() => {
    ...
    let speedModifier = campaignMode?.enabled && campaignMode.levelConfig 
    ...
    slowMotionState.current = SlowMotion.createInitialSlowMotionState(upgradeEffects.slowMotionUses);
    ...
    let spawnRateModifier = campaignMode?.enabled && campaignMode.levelConfig
    ...
    if (dailyChallengeMode?.enabled && dailyChallengeMode.config?.modifiers.doubleObstacles) {
    ...
    if (zenMode?.enabled) {
      zenModeState.current = ZenMode.activateZenMode(ZenMode.createInitialZenModeState());
  ...
  const createNearMissEffect = (orbX: number, orbY: number, closestPoint: { x: number; y: number }, isStreakBonus: boolean) => {
    // Theme System Integration - Requirements 5.1, 5.2, 5.3
    const accentColor = getColor('accent');
    ...
    ParticleSystem.emitSpark(closestPoint.x, closestPoint.y, [accentColor, '#FFD700', '#FFA500']);
  ...
  const handleJoystickStartNative = useCallback((e: TouchEvent | MouseEvent) => {
    if (gameState !== GameState.PLAYING) return;
    e.preventDefault();
    e.stopPropagation();
  ...
  const handleJoystickMoveNative = useCallback((e: TouchEvent | MouseEvent) => {
    if (!joystickRef.current.active || gameState !== GameState.PLAYING) return;
    e.preventDefault();
    e.stopPropagation();
    ...
    const maxOffset = 50;
    let offset = Math.max(-maxOffset, Math.min(maxOffset, clientY - joystickRef.current.startY));
    
    // Daily Challenge Mode: Apply inverted controls - Requirements 8.2
    if (dailyChallengeMode?.enabled && dailyChallengeMode.config?.modifiers.invertedControls) {
    ...
    const newY = Math.max(0, Math.min(1, currentY + normalizedOffset * moveSpeed));
  ...
  const handleSwapButtonNative = useCallback((e: TouchEvent | MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    triggerSwap();
  }, [triggerSwap]);
  ...
  const handleTouchControlMove = useCallback((e: TouchEvent) => {
    ...
    const touch = Array.from(e.touches).find(t => t.identifier === touchControlRef.current.touchId);
    if (!touch) return;
  ...
  const handleTouchControlEnd = useCallback((e: TouchEvent) => {
    ...
    const touchEnded = !Array.from(e.touches).some(t => t.identifier === touchControlRef.current.touchId);
  ...
  }, [handleTouchControlStart, handleTouchControlMove, handleTouchControlEnd, isMobile, gameState]);
  ...
  useEffect(() => {
    if (gameState === GameState.MENU) {
      ...
      if (canvas && ctx) {
        ...
        ctx.beginPath(); ctx.moveTo(cx, cy - 40); ctx.lineTo(cx, cy + 40);
        ctx.strokeStyle = getColor('connector'); ctx.lineWidth = 4; ctx.stroke();
        
        ctx.beginPath(); ctx.arc(cx, cy - 40, 15, 0, Math.PI * 2); ctx.fillStyle = getColor('topOrb'); ctx.fill();
        ctx.beginPath(); ctx.arc(cx, cy + 40, 15, 0, Math.PI * 2); ctx.fillStyle = getColor('bottomOrb'); ctx.fill();
      }
      return;
    }
    ...
    const loop = () => {
      ...
      if (slowMotionActive && !slowMotionState.current.isActive && slowMotionState.current.usesRemaining >= 0) {
        slowMotionState.current = SlowMotion.activateSlowMotion(slowMotionState.current, gameTime);
        slowMotionStartTime.current = gameTime;
      }
      
      // Update slow motion state
      if (slowMotionState.current.isActive) {
        slowMotionState.current = SlowMotion.updateSlowMotion(slowMotionState.current, gameTime);
      ...
      const slowMotionMultiplier = SlowMotion.getSpeedMultiplier(slowMotionState.current);
      ...
      const targetLen = Math.min(
          ...
          INITIAL_CONFIG.minConnectorLength + (score.current * INITIAL_CONFIG.connectorGrowthRate)
      );
      currentConnectorLength.current += (targetLen - currentConnectorLength.current) * 0.01;
      ...
      const midlineEnabled = (!campaignMode?.enabled || campaignMode.levelConfig?.mechanics.midline !== false)
        && !(dailyChallengeMode?.enabled && dailyChallengeMode.config?.modifiers.noMidline);
      ...
      const currentAmplitude = midlineEnabled
        ? calculateDynamicAmplitude(midlineConfig.baseAmplitude, score.current, midlineConfig)
        : 0;
      ...
      const normalizedOffset = calculateNormalizedOffset(height, currentMidlineY);
      ...
      const dynamicBounds = calculateMovementBounds(currentMidlineY, currentConnectorLength.current, radius, height);
      
      // Also calculate normal bounds for critical space detection
      const normalBounds = calculateNormalBounds(height, currentConnectorLength.current, radius);
      ...
      if (!shiftState.current.overdriveActive) {
        ...
        const spawnProbability = ShiftProtocol.calculateSpawnProbability(score.current);
        ...
        if (framesSinceCollectibleSpawn.current >= 60) {
          ...
          if (Math.random() < spawnProbability) {
            ...
            const nextLetterIndex = ShiftProtocol.selectNextLetter(shiftState.current.collectedMask);
            ...
            if (nextLetterIndex !== -1) {
              ...
              const oscillationAmplitude = SHIFT_CONFIG.oscillation.defaultAmplitude;
              ...
              if (safeMaxY > safeMinY) {
                ...
                const newCollectible: Collectible = {
                  ...
                  oscillationFrequency: SHIFT_CONFIG.oscillation.defaultFrequency,
                  isCollected: false,
                };
      ...
      if (!gravityState.current.warningActive && !gravityState.current.isFlipped) {
        // Check if we should start warning (500ms before flip)
        if (shouldTriggerFlip(score.current, gravityState.current.lastFlipTime, currentTime)) {
          // Random chance to trigger warning (to add unpredictability)
          if (Math.random() < 0.005) { // ~0.5% chance per frame when eligible
      ...
      if (gravityState.current.warningActive) {
        const warningElapsed = currentTime - gravityState.current.warningStartTime;
      ...
      if (gravityState.current.isFlipped) {
        ...
        if (flipDuration >= GRAVITY_CONFIG.minTimeBetweenFlips) {
          ...
          if (Math.random() < 0.003) { // Lower chance to flip back
            gravityState.current.warningActive = true;
            gravityState.current.warningStartTime = currentTime;
          }
        }
      ...
      // --- DAILY RITUALS TRACKING - Requirements 3.7, 3.8, 3.9, 3.10 ---
      if (ritualTracking) {
        const deltaTime = lastRitualUpdateTime.current > 0 
        ...
        if (speedKmh >= 100 && deltaTime > 0) {
          ...
          ritualTracking.onSpeedSurvival?.(Math.floor(speedSurvivalTime.current));
        }
        
        // Track no-swap survival time
        const timeSinceLastSwap = (currentTime - lastSwapTimeForRitual.current) / 1000;
        if (timeSinceLastSwap > noSwapSurvivalTime.current) {
          noSwapSurvivalTime.current = timeSinceLastSwap;
          ritualTracking.onNoSwapSurvival?.(Math.floor(noSwapSurvivalTime.current));
        }
      ...
      // Theme System Integration - Requirements 5.1, 5.2, 5.3
      const whiteOrb = { x: whiteOrbX, y: whiteOrbY, radius: INITIAL_CONFIG.orbRadius, color: getColor('topOrb') };
      const blackOrb = { x: blackOrbX, y: blackOrbY, radius: INITIAL_CONFIG.orbRadius, color: getColor('bottomOrb') };
      ...
      ParticleSystem.emitTrail(whiteOrbX - 5, whiteOrbY, speed.current, [whiteOrb.color, getColor('accent')]);
      ...
      collectibles.current = collectibles.current.filter(collectible => {
        ...
        if (whiteCollision || blackCollision) {
          ...
          const letterIndex = ShiftProtocol.TARGET_WORD.indexOf(collectible.value);
          
          if (letterIndex !== -1) {
            // Mark letter as collected - Requirements 9.1
            shiftState.current = ShiftProtocol.collectLetter(shiftState.current, letterIndex);
            ...
            if (ShiftProtocol.checkOverdriveActivation(shiftState.current.collectedMask)) {
              shiftState.current = ShiftProtocol.activateOverdrive(shiftState.current);
              ...
              if (resonanceState.current.isActive && !resonanceState.current.isPaused) {
                resonanceState.current = ResonanceSystem.pauseResonance(resonanceState.current);
              }
              
              // Visual feedback for Overdrive activation
              ParticleSystem.emitBurst(playerX, playerY.current * height, ['#FFD700', '#FF00FF', '#00F0FF']);
              scorePopups.current.push({
                x: width / 2,
                y: height / 2 - 80,
      ...
      if (restoreMode?.enabled && !restoreState.current.hasBeenUsed) {
        ...
        snapshotBuffer.current = RestoreSystem.pushSnapshot(snapshotBuffer.current, snapshot);
        ...
        restoreState.current = RestoreSystem.recordSnapshot(restoreState.current, legacySnapshot);
      }
      ...
      obstacles.current.forEach(obs => {
        ...
        if (!obs.passed && obs.x + obs.width < playerX - INITIAL_CONFIG.orbRadius) {
          ...
          if (obs.lane === 'top') {
              ...
              const rhythmResult = checkRhythmTiming(currentTime, rhythmState.current);
              ...
              onRhythmStateUpdate?.(rhythmState.current.activeMultiplier, rhythmState.current.streakCount);
              ...
              if (!resonanceState.current.isActive && 
                ...
                ParticleSystem.emitBurst(playerX, playerY.current * height, ['#00F0FF', '#FF00FF', '#FFFFFF']);
              }
              
              // Chromatic Aberration - Requirements 11.1, 11.2: Trigger based on rhythm streak
              ChromaticAberration.setStreakLevel(rhythmState.current.activeMultiplier);
              ...
              if (!zenMode?.enabled) {
                const finalScore = Math.floor(earnedScore * scoreMultiplierUpgrade.current);
              ...
              if (campaignMode?.enabled && campaignMode.targetScore && !levelCompleted.current) {
        ...
        const zenModeInvincible = zenMode?.enabled && ZenMode.isRespawnInvincible(zenModeState.current, Date.now());
        
        // Check Overdrive invulnerability - Requirements 4.3
        const overdriveInvincible = ShiftProtocol.isInvulnerableDuringOverdrive(shiftState.current);
        ...
        if (overdriveInvincible) {
          ...
          const whiteCollision = checkCollision(whiteOrbPos, whiteOrb.radius, obs);
          const blackCollision = checkCollision(blackOrbPos, blackOrb.radius, obs);
          
          if (whiteCollision || blackCollision) {
            // Destroy obstacle - Requirements 4.4
            const { shouldDestroy } = ShiftProtocol.handleOverdriveCollision(shiftState.current);
            
            if (shouldDestroy) {
              // Create destruction particle effect
              createExplosion(obs.x + obs.width / 2, obs.y + obs.height / 2, '#FFD700');
              ParticleSystem.emitBurst(obs.x + obs.width / 2, obs.y + obs.height / 2, ['#FFD700', '#FF00FF', '#00F0FF']);
        ...
        if (isPhasing.current || gravityState.current.isInvincible || zenModeInvincible || postRestoreInvincibleActive || pendingRestore.current || isRestoreAnimating.current) return;
        ...
        const whiteMicroPhasing = shouldApplyMicroPhasing(whiteOrb.y, currentMidlineY);
        const blackMicroPhasing = shouldApplyMicroPhasing(blackOrb.y, currentMidlineY);
        ...
        if (whiteCollision) {
             ...
             if (whiteMicroPhasing) {
                 ...
                 if (resonanceState.current.isActive) {
                     const { result, newState } = ResonanceSystem.handleCollision(resonanceState.current);
                     resonanceState.current = newState;
                     
                     if (result.destroyed) {
                         // Requirements 1.6: Trigger explosion particle effect at obstacle position
                         createExplosion(obs.x + obs.width / 2, obs.y + obs.height / 2, '#00F0FF');
                         ParticleSystem.emitBurst(obs.x + obs.width / 2, obs.y + obs.height / 2, ['#00F0FF', '#FF00FF', '#FFFFFF']);
        ...
        if (!whiteCollision && !blackCollision && !obs.nearMissChecked) {
          ...
          if (obs.polarity === 'black') {
            const whiteNearMiss = checkNearMiss(whiteOrbPos, whiteOrb.radius, obs);
            if (whiteNearMiss.isNearMiss) {
              ...
              // Zen Mode: Skip score tracking - Requirements 9.1
              if (!zenMode?.enabled) {
                const finalBonus = Math.floor(totalBonus * scoreMultiplierUpgrade.current);
              ...
              createNearMissEffect(whiteOrb.x, whiteOrb.y, whiteNearMiss.closestPoint, streakResult.streakBonusAwarded);
            }
          }
          
          // Check black orb near miss with white obstacles (dangerous ones)
          if (obs.polarity === 'white') {
            const blackNearMiss = checkNearMiss(blackOrbPos, blackOrb.radius, obs);
            if (blackNearMiss.isNearMiss) {
              ...
              // Zen Mode: Skip score tracking - Requirements 9.1
              if (!zenMode?.enabled) {
                const finalBonusBlack = Math.floor(totalBonus * scoreMultiplierUpgrade.current);
              ...
              // Create visual effects (will be implemented in 5.3)
              createNearMissEffect(blackOrb.x, blackOrb.y, blackNearMiss.closestPoint, streakResult.streakBonusAwarded);
            }
          }
        }
      });
      ...
      if (ghostRacerMode?.enabled && ghostRacerState.current.isRecording) {
        ...
        const timeSinceStart = gameTime - ghostRacerState.current.recordingStartTime;
        ...
        if (timeSinceStart - lastGhostRecordTime.current >= recordingInterval) {
          ...
          ghostRacerState.current = GhostRacer.recordFrame(ghostRacerState.current, frame);
      ...
      if (shiftState.current.overdriveActive) {
        ...
        shiftState.current = ShiftProtocol.updateOverdrive(shiftState.current, overdriveDeltaTime);
        
        // Update core rotation for visual effect
        shiftState.current = {
          ...shiftState.current,
          coreRotation: (shiftState.current.coreRotation + 0.05) % (Math.PI * 2)
        };
        ...
        if (!shiftState.current.overdriveActive) {
          ...
          // Requirements: Overdrive > Resonance (Override pattern)
          if (resonanceState.current.isActive && resonanceState.current.isPaused) {
            resonanceState.current = ResonanceSystem.resumeResonance(resonanceState.current);
      ...
      scorePopups.current = scorePopups.current.filter(popup => popup.life > 0);
      ...
      visualEffects.current = visualEffects.current.filter(effect => effect.life > 0);
      ...
      const shakeMultiplier = zenMode?.enabled ? ZenMode.getShakeMultiplier(zenModeState.current) : 1.0;
      ctx.save();
      ctx.translate(shakeOffset.x * shakeMultiplier, shakeOffset.y * shakeMultiplier);

      // Theme System Integration - Requirements 5.1, 5.2, 5.3
      // Backgrounds - Requirements 2.2, 4.3: Swap colors when gravity is flipped, use dynamic midline
      let topBgColor = gravityState.current.isFlipped ? getColor('bottomBg') : getColor('topBg');
      let bottomBgColor = gravityState.current.isFlipped ? getColor('topBg') : getColor('bottomBg');
      
      // Resonance Mode: Color Inversion - Requirements 1.2, 1.9
      const resonanceInversion = ResonanceSystem.getColorInversion(resonanceState.current);
      ...
      ctx.beginPath(); ctx.moveTo(0, currentMidlineY); ctx.lineTo(width, currentMidlineY);
      ...
      if (tension > 0.1) {
        ...
        const tensionColor = tension > 0.7 ? '#FF4444' : tension > 0.4 ? '#FFAA00' : accentColor;
        const lineWidth = 2 + tension * 6; // 2-8 arası kalınlık
        
        // Titreşim efekti - gerilim yüksekken çizgi hafifçe titrer
        const shake = tension > 0.5 ? Math.sin(Date.now() * 0.05) * tension * 3 : 0;
      ...
      if (gravityState.current.warningActive) {
        ...
        const pulseIntensity = Math.sin(warningProgress * Math.PI * 4) * 0.5 + 0.5; // Pulsing effect
      ...
      if (resonanceState.current.isActive) {
        const remainingSeconds = Math.ceil(resonanceState.current.remainingTime / 1000);
        const timerProgress = resonanceState.current.remainingTime / ResonanceSystem.RESONANCE_CONFIG.duration;
        ...
        ctx.fillStyle = '#00F0FF';
        ctx.textAlign = 'center';
        ctx.fillText(`⚡ HARMONIC RESONANCE ⚡ ${remainingSeconds}s`, width / 2, barY - 10);
        
        // Destroyed count
        if (resonanceState.current.obstaclesDestroyed > 0) {
          ctx.font = 'bold 12px Arial';
          ctx.fillText(`💥 ${resonanceState.current.obstaclesDestroyed} destroyed (+${resonanceState.current.bonusScore})`, width / 2, barY + 25);
        }
      }
      ...
      if (obstacles.current.length > 0 && framesSinceSpawn.current % 60 === 0) {
        console.log('[RENDER] Drawing', obstacles.current.length, 'obstacles. First obs:', obstacles.current[0]?.x, obstacles.current[0]?.y);
      }
      ...
      obstacles.current.forEach(obs => {
        ...
        if (hasEffect('pixelated')) {
          ...
          // Top-right corner
          ctx.fillRect(obs.x + obs.width - pixelSize, obs.y, pixelSize, pixelSize);
          // Bottom-left corner
          ctx.fillRect(obs.x, obs.y + obs.height - pixelSize, pixelSize, pixelSize);
          // Bottom-right corner
          ctx.fillRect(obs.x + obs.width - pixelSize, obs.y + obs.height - pixelSize, pixelSize, pixelSize);
        }
      ...
      const gradient = ctx.createLinearGradient(whiteOrb.x, whiteOrb.y, blackOrb.x, blackOrb.y);
      ...
      const drawOrb = (orb: {x:number, y:number, radius:number, color:string}, isWhite: boolean) => {
          ...
          if (resonanceState.current.isActive || resonanceInversion.factor > 0) {
            ...
            ctx.arc(orb.x, orb.y, orb.radius, 0, Math.PI * 2);
            ctx.lineWidth = 2;
            ctx.strokeStyle = isWhite ? getColor('bottomOrb') : getColor('topOrb');
            ctx.stroke();
      ...
      if (ghostRacerMode?.enabled && savedGhostTimeline.current) {
        const timeSinceStart = gameTime - ghostRacerState.current.recordingStartTime;
        const ghostPosition = GhostRacer.getGhostPosition(savedGhostTimeline.current, timeSinceStart);
        ...
        if (ghostPosition) {
          ...
          if (GhostRacer.isPlayerAhead(score.current, timeSinceStart, savedGhostTimeline.current)) {
            GhostRacer.renderAheadIndicator(ctx, playerX, playerY.current * height);
          }
        }
      }
      ...
      if (postRestoreInvincible.current) {
        const timeSinceRestore = Date.now() - postRestoreStartTime.current;
        const remainingTime = POST_RESTORE_INVINCIBILITY_DURATION - timeSinceRestore;
      ...
      const visualIntensity = zenMode?.enabled ? ZenMode.getVisualIntensity(zenModeState.current) : 1.0;
      ...
      particles.current.forEach(p => {
        ...
        ctx.beginPath(); ctx.arc(p.x, p.y, randomRange(2, 5), 0, Math.PI * 2); ctx.fill();
        ctx.globalAlpha = 1.0;
      });
      ...
      scorePopups.current.forEach(popup => {
        ...
        ctx.font = popup.text === 'PERFECT DODGE!' ? 'bold 16px Arial' : 'bold 14px Arial';
      ...
      if (collisionDetected) {
        ...
        if (zenMode?.enabled && zenModeState.current.isActive) {
          const collisionTime = Date.now();
          const { newState, shouldRespawn } = ZenMode.handleZenModeCollision(zenModeState.current, collisionTime);
        ...
        } else if (restoreMode?.enabled && !restoreState.current.hasBeenUsed && !pendingRestore.current) {
          ...
          const canRestore = RestoreSystem.canRestore(restoreState.current, useGameStore.getState().echoShards);
          ...
          if (ghostRacerMode?.enabled && ghostRacerState.current.isRecording) {
            ...
            ghostRacerState.current = GhostRacer.stopRecording(ghostRacerState.current, finalScore);
            
            // Check if new high score and save - Requirements 15.4
            if (GhostRacer.isNewHighScore(finalScore, savedGhostTimeline.current)) {
          ...
          snapshotBuffer.current = RestoreSystem.clearSnapshotBuffer(snapshotBuffer.current);
  ...
  }, [restoreRequested, restoreMode, onRestoreStateUpdate, onScoreUpdate, setGameSpeedDisplay, onGameOver]);

  return (
    <div ref={containerRef} className="absolute inset-0 w-full h-full overflow-hidden cursor-pointer touch-none select-none">
