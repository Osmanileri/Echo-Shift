...
import { BookOpen, CheckCircle, ChevronLeft, ChevronRight, Ghost, Lock, Play, Star, Trophy, Waves, X, Zap } from 'lucide-react';
...
interface LevelMapProps {
  ...
  /** Callback to start interactive tutorial - Level 0 */
      ...
      <div className="relative w-full max-w-2xl mx-4 max-h-[90vh] bg-gradient-to-b from-gray-900/90 to-black/90 rounded-2xl border border-white/10 overflow-hidden">
        ...
        <div className="flex items-center justify-between p-4 border-b border-white/10">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            {/* Tutorial Button - Access tutorial anytime */}
            {onStartTutorial && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-500/20 hover:bg-purple-500/30 rounded-full border border-purple-500/50 transition-all"
                title="Eğitim Bölümü"
              >
                <BookOpen className="w-4 h-4 text-purple-400" />
                <span className="text-xs font-medium text-purple-400 hidden sm:inline">Eğitim</span>
              </button>
            )}
        ...
        <div className="flex items-center justify-between p-4 border-t border-white/10">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all ${currentPage === 0
          ...
          <div className="flex items-center gap-2">
            {Array.from({ length: TOTAL_PAGES }).map((_, index) => (
              <button
                ...
                className={`w-2.5 h-2.5 rounded-full transition-all ${index === currentPage
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all ${currentPage === TOTAL_PAGES - 1
                ? 'text-white/30 cursor-not-allowed'
                : 'text-white hover:bg-white/10'
              }`}
          >
  ...
  return (
    <>
      ...
      >
        ...
        <div className={`text-lg font-bold ${isLocked && !showLockRemoval
            ? 'text-gray-500'
            : isNextToPlay
              ? 'text-cyan-400'
              : isHighlighted
        ...
        {(chapterUnlocked || showLockRemoval) && (
          <div className="flex items-center gap-0.5 mt-1">
            {[1, 2, 3].map((starNum) => (
              <Star
                key={starNum}
                className={`w-3 h-3 ${starNum <= stars
