...
import { Play, RotateCcw, Trophy, ArrowRightLeft, Pause, Home, PlayCircle, ChevronDown, Gem, ShoppingBag, Clock } from 'lucide-react';
      ...
      <>
        <div className="absolute top-0 left-0 w-full p-4 flex justify-between items-start pointer-events-none z-10">
          <div className="flex flex-col items-start">
            ...
            <span className="text-[10px] md:text-xs text-gray-400 font-bold mt-1 uppercase tracking-widest">Skor</span>
          </div>
          <button 
            onClick={(e) => { e.stopPropagation(); handleButtonClick(onPause); }}
        ...
        <div className="absolute top-20 right-4 pointer-events-none z-10 flex flex-col gap-2 items-end">
          ...
          {rhythmStreak > 0 && (
            <div className="flex items-center gap-2 px-2 py-1 bg-cyan-500/10 rounded-md border border-cyan-500/30 backdrop-blur-sm">
              <span className="text-[10px] md:text-xs text-cyan-400/70 uppercase tracking-wider">Ritim</span>
              <span className="text-sm md:text-base font-bold text-cyan-400">{rhythmStreak}</span>
            </div>
          )}
          ...
          {nearMissStreak > 0 && (
            <div className="flex items-center gap-2 px-2 py-1 bg-purple-500/10 rounded-md border border-purple-500/30 backdrop-blur-sm">
              <span className="text-[10px] md:text-xs text-purple-400/70 uppercase tracking-wider">Dodge</span>
              <span className="text-sm md:text-base font-bold text-purple-400">{nearMissStreak}</span>
            </div>
          )}
        </div>
        ...
        <div className="absolute bottom-6 right-6 pointer-events-none z-10 opacity-70">
          <div className="flex flex-col items-center gap-2">
            ...
            <span className="text-[9px] md:text-[10px] font-bold text-cyan-400 tracking-widest uppercase">Dokun: Değiştir</span>
          </div>
        </div>
        ...
        {slowMotionUsesRemaining > 0 && onActivateSlowMotion && (
          <div className="absolute bottom-6 left-6 z-10">
            <button
              onClick={(e) => { e.stopPropagation(); handleButtonClick(onActivateSlowMotion); }}
            ...
            >
              <div className={`w-14 h-14 md:w-16 md:h-16 rounded-full border-2 flex items-center justify-center backdrop-blur-sm transition-all duration-300 ${
                slowMotionActive 
                ...
                <Clock className={`w-6 h-6 md:w-8 md:h-8 ${slowMotionActive ? 'text-purple-300 animate-pulse' : 'text-purple-400'}`} />
              </div>
              <span className="text-[9px] md:text-[10px] font-bold text-purple-400 tracking-widest uppercase">
                {slowMotionActive ? 'AKTİF' : `YAVAŞLAT (${slowMotionUsesRemaining})`}
              </span>
            </button>
          </div>
        )}
  ...
  if (gameState === GameState.MENU) {
    ...
    return (
      <div className="absolute inset-0 flex flex-col items-center justify-between bg-gradient-to-b from-black via-gray-950 to-black z-20 text-white overflow-hidden">
        ...
        <div className={`flex flex-col items-center pt-6 sm:pt-10 md:pt-16 transition-all duration-1000 ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-10'}`}>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 px-4">
            {highScore > 0 && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 rounded-full border border-white/10 backdrop-blur-sm">
                ...
                <span className="text-[10px] sm:text-xs text-white/70 tracking-wider">{highScore}</span>
              </div>
            )}
            ...
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-cyan-500/10 rounded-full border border-cyan-500/30 backdrop-blur-sm">
              ...
              <span className="text-[10px] sm:text-xs text-cyan-400 tracking-wider font-bold">{echoShards.toLocaleString()}</span>
            </div>
            ...
            {onOpenShop && (
              ...
              >
                ...
                <span className="text-[10px] sm:text-xs text-white/70 tracking-wider font-medium">MAĞAZA</span>
              </button>
            )}
          </div>
          ...
          {onOpenDailyChallenge && (
            ...
            >
              ...
              <span className="text-[10px] sm:text-xs text-orange-400 tracking-wider font-bold">GÜNLÜK MEYDAN OKUMA</span>
            </button>
          )}
        </div>

        {/* Center - Animated Demo - Compact */}
        <div className={`flex-1 flex flex-col items-center justify-center w-full max-w-sm px-4 transition-all duration-1000 delay-300 ${showContent ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
          ...
          <div className="w-full space-y-2">
            <div className="flex items-center gap-3 p-2.5 bg-white/5 rounded-lg border border-white/10 backdrop-blur-sm">
              ...
              <div>
                ...
                <p className="text-white/50 text-[10px]">Yukarı ve aşağı hareket et</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 p-2.5 bg-white/5 rounded-lg border border-white/10 backdrop-blur-sm">
              ...
              <div>
                ...
                <p className="text-white/50 text-[10px]">Topların yerini değiştir</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 p-2.5 bg-white/5 rounded-lg border border-white/10 backdrop-blur-sm">
              ...
              <div>
                ...
                <p className="text-white/50 text-[10px]">Aynı renkten geç, zıt renkten kaç</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className={`w-full px-4 pb-6 sm:pb-8 transition-all duration-1000 delay-500 ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <button 
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center z-30 text-white overflow-hidden px-4">
        ...
        <div className="relative z-10 flex flex-col items-center w-full max-w-xs">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-black mb-1 tracking-[0.15em] text-white relative">
            OYUN
            <span className="absolute -right-1 top-0 text-red-500 opacity-50 blur-sm">OYUN</span>
          </h2>
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-black mb-4 tracking-[0.15em] text-red-500 relative">
            BİTTİ
            <span className="absolute -left-1 top-0 text-white opacity-30 blur-sm">BİTTİ</span>
          </h2>
          ...
          <div className="flex flex-col items-center gap-1 mb-4 p-4 bg-white/5 rounded-xl border border-white/10 backdrop-blur-sm w-full">
            ...
            <span className="text-[10px] sm:text-xs text-white/50 uppercase tracking-[0.2em]">Final Skor</span>
          </div>
          ...
          {score >= highScore && score > 0 && (
            <div className="flex items-center gap-2 mb-3 px-4 py-2 bg-gradient-to-r from-yellow-500/20 to-orange-500/20 rounded-full border border-yellow-500/30 animate-pulse">
              ...
              <span className="text-xs font-bold text-yellow-500 tracking-wider">YENİ REKOR!</span>
              <Trophy className="w-4 h-4 text-yellow-500" />
            </div>
          )}
          ...
          {earnedShards > 0 && (
            <div className="flex items-center gap-2 mb-4 px-4 py-2 bg-gradient-to-r from-cyan-500/20 to-cyan-400/20 rounded-full border border-cyan-500/30">
              ...
              <span className="text-xs font-bold text-cyan-400 tracking-wider">+{earnedShards} SHARDS</span>
            </div>
          )}
