...
describe('Distance Tracking Properties - Campaign Update v2.5', () => {
   ...
   * increase currentDistance by `speed * deltaTime`.
   */
  test('Property 3: Distance accumulation', () => {
    fc.assert(
      fc.property(
        // Target distance
        fc.integer({ min: 100, max: 10000 }),
        // Delta time in seconds (positive, reasonable range)
        fc.float({ min: Math.fround(0.001), max: Math.fround(1.0), noNaN: true }),
        // Speed in meters per second (positive)
        ...
        (targetDistance, deltaTime, speed) => {
          ...
          // Distance should increase by speed * deltaTime
          const expectedIncrease = speed * deltaTime;
          expect(newDistance).toBeCloseTo(initialDistance + expectedIncrease, 5);
   ...
   */
  test('Property 3: Distance accumulation is additive', () => {
    fc.assert(
      fc.property(
        // Target distance
        ...
        (targetDistance, updates) => {
          ...
          // Final distance should equal sum of all increments
          expect(tracker.getCurrentDistance()).toBeCloseTo(expectedTotal, 5);
    ...
    fc.assert(
      fc.property(
        ...
        (targetDistance) => {
          ...
          // Test just below 80%
          const justBelow = targetDistance * 0.799;
...
describe('Distance Tracker Edge Cases', () => {
  ...
  test('Progress percentage is capped at 100', () => {
    ...
    expect(state.currentDistance).toBe(1500);
  });
