...
import { GameState, Obstacle, Particle, NearMissState, ScorePopup, VisualEffect } from '../types';
...
const GameEngine: React.FC<GameEngineProps> = ({ 
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      // Restore System: Record snapshot for potential rewind - Requirements 2.5
      if (restoreMode?.enabled && !restoreState.current.hasBeenUsed) {
        const snapshot = RestoreSystem.createSnapshot(
          Date.now(),
          score.current,
          playerY.current,
          isSwapped.current,
        ...
        restoreState.current = RestoreSystem.recordSnapshot(restoreState.current, snapshot);
      }
      ...
      obstacles.current.forEach(obs => {
        ...
        // --- COLLISION LOGIC ---
        // Skip collision if phasing (swap), gravity invincibility, Zen Mode respawn invincibility, or post-restore invincibility is active
        // Requirements 2.7, 9.2
