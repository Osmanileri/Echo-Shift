...
import { UPGRADES, Upgrade, getUpgradeById, getUpgradeCost as getUpgradeCostFromData, getUpgradeEffect as getUpgradeEffectFromData } from '../data/upgrades';
 ...
 * Requirements: 6.5
 */
export function canPurchaseUpgrade(upgradeId: string, currentLevel: number, balance: number): boolean {
  const upgrade = getUpgradeById(upgradeId);
...
export function getAllUpgradeStates(): UpgradeState[] {
  ...
  return UPGRADES.map(upgrade => {
    ...
    return {
      ...
      nextEffect: isMaxed ? getUpgradeEffect(upgrade.id, currentLevel) : getUpgradeEffect(upgrade.id, currentLevel + 1),
      isMaxed,
    };
  });
}
